...
public class ProfessorSistemasDao {
...
public static Professor autenticar(String nomeDeUsuario, String senha){
		...
		try {
			Professor professor = (Professor) em.createQuery("SELECT p FROM ProfessorSistemas p WHERE p.nomeDeUsuario = :nomeDeUsuario and p.senha = :senha")
	...
	public List<ProfessorSistemas> getLista(){
		
		EntityManager em = ConexaoJPA.getEntityManager();
		
		Query consulta = em.createQuery("SELECT p FROM ProfessorSistemas p");
